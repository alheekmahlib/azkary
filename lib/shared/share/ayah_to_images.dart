...
/// Share Ayah
Future<Uint8List> createVerseImage(String zkrText, zkrCategory, zkrDescription) async {
...
Future<Uint8List> createVerseWithTranslateImage(BuildContext context,
  ...
  // Set the text position
  final textX = (canvasWidth - textZkr.width) / 2; // Center the text horizontally
  final textY = image2Y + image2Height + padding + 20; // Position the text below the second image
      ...
      text: TextSpan(
        ...
        style: TextStyle(
            ...
            color: ThemeProvider.themeOf(context).id == 'dark'
                ? Colors.white
                : const Color(0xff263A29),
            backgroundColor: const Color(0xff41644A).withOpacity(.2),
        ),
      ),
      textDirection: TextDirection.rtl,
      ...
      text: TextSpan(
        ...
        style: TextStyle(
            ...
            color: ThemeProvider.themeOf(context).id == 'dark'
                ? Colors.white
                : const Color(0xff263A29),
            backgroundColor: const Color(0xff41644A).withOpacity(.3)
        ),
      ),
      textDirection: TextDirection.rtl,
      textAlign: TextAlign.justify
  ...
  final descriptionZkrY = textY + textZkr.height + padding + referenceZkr.height + 10;
  ...
  // Set the canvas width and height
  canvasHeight = textZkr.height + descriptionZkr.height + referenceZkr.height + imageHeight + image2Height + image3Height * padding;
  final countZkrY = zkrDescription == ""
      ? textY + textZkr.height + referenceZkr.height + padding + 5
      : textY + textZkr.height + descriptionZkr.height + referenceZkr.height + padding + 40;
  // Check if the total height is less than the minimum height
  if (canvasHeight < minHeight) {
    canvasHeight = minHeight;
  }
  ...
  // Update the canvas height
  canvasHeight += image3Height + descriptionZkr.height + referenceZkr.height + 30 + padding;
  ...
  final picture = pictureRecorder.endRecording();
  final imgWidth = canvasWidth.toInt(); // Use canvasWidth instead of (textZkr.width + 2 * padding)
  final imgHeight = zkrDescription == ""
      ? (textZkr.height + referenceZkr.height + imageHeight + image3Height + 5.6 * padding).toInt()
      : (textZkr.height + descriptionZkr.height + referenceZkr.height + imageHeight + image3Height + 7.1 * padding).toInt();
...
Future<void> shareVerseWithTranslate(BuildContext context,
    String zkrText, zkrCategory, zkrDescription, zkrCount, azkarReference) async {
  final imageData = await createVerseWithTranslateImage(context,
    zkrText, zkrCategory, zkrDescription, zkrCount, azkarReference);
...
Future<void> shareVerse(BuildContext context, String zkrText, zkrCategory, zkrDescription) async {
  final imageData2 = await createVerseImage(zkrText, zkrCategory, zkrDescription);
...
void showVerseOptionsBottomSheet(BuildContext context,
  ...
  Uint8List imageData = await createVerseWithTranslateImage(context,
      zkrText, zkrCategory, zkrDescription, zkrCount, azkarReference);
